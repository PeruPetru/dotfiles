...
int main( int argc, char *argv[] ){
    ...
    BRIGHTNESS_FILE_PTR = fopen("/sys/class/backlight/amdgpu_bl1/brightness", "r+");
    ...
    for( int i = 1; i < argc; i += 1 ){
        if( !strcmp(argv[i], "-s") || !strcmp(argv[i], "--set") || !strcmp(argv[i], "set") ) {
            ...
            if( value <   0 ) value =   0;
            BRIGHTNESS_FILE_PTR = freopen("/sys/class/backlight/amdgpu_bl1/brightness", "w", BRIGHTNESS_FILE_PTR);
            sprintf(dunstifyBuffer, "dunstify \"Brightness: %.1f\" -t 1250 -r 19066", value / 255.0f);
            ...
            sprintf(dunstifyBuffer, "dunstify \"Brightness: %.1f\" -t 1250 -r 19066", brightness / 255.0f);
            system(dunstifyBuffer);
